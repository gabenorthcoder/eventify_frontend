...
export default function RegisterSuperAdmin(props: { disableCustomTheme?: boolean }) {
...
const [roleErrorMessage, setRoleErrorMessage] = React.useState('');
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        const roleMapping: Record<Role, number> = { ADMIN: 0, STAFF: 1, USER: 2 };
        await apiService.superAdminRegister({ email, password, firstName, lastName, role: roleMapping[role] }, token);
  
      setSuccessMessage(`${role === "ADMIN" ? "Admin" : role === "STAFF" ? "Staff" : "User"} Registration successful!`);
      setTimeout(() => {
      ...
      if (err.response && err.response.status === 400 && err.response.data.message) {
        setError(err.response.data.message || `${role === "ADMIN" ? "Admin" : role === "STAFF" ? "Staff" : "User"} Registration failed`);
      } else {
